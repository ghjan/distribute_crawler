...
def AFTER_SETUP():
    ...
    for (collection, keystr) in COLLECTION_KEYS.items():
    ...
    for k, v in list(INDEX.items()):
        for key, kwargs in list(v.items()):
            conn[ShardMONGODB_DB][k].ensure_index(list(key) if type(key) == tuple else key, **kwargs)
...
def printer():
    ...
    while fds:
        (files, _, errors) = select(list(fds.keys()), [], list(fds.keys()), 1)
